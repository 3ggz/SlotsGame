...
run('jackpot Firestore listener backs off on resource exhaustion', () => {
  ...
  assert(src.includes('const JACKPOTS_LIVE_ALLOWED'), 'mobile Dragon Tree should be able to skip live jackpot sync');
  assert(src.includes('if (!JACKPOTS_LIVE_ALLOWED) return;'), 'live jackpot startup should no-op when disabled for performance');
  assert(dragonTreeHtml.includes('casino-jackpots.js?v=77'), 'Dragon Tree should bypass stale cached jackpot scripts');
});
