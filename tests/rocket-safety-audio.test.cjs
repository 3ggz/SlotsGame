...
assertMatch(/const safetyLocked\s*=\s*betSafetyLocked\(\);[\s\S]*?const canPlace\s*=\s*phase === 'betting' && !localBetInRound && !safetyLocked/, 'Rocket disables place-bet controls while crash is settling');
